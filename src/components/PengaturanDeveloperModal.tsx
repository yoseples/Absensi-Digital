...
import { AppConfig, DomainTenantConfig, FirebaseAppConfig, SupabaseAppConfig } from '../types';
...
export const PengaturanDeveloperModal: React.FC<PengaturanDeveloperModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'sekolah' | 'branding' | 'login_screen' | 'akademik' | 'absensi' | 'sistem' | 'cpanel' | 'firebase' | 'supabase'>('sekolah');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const currentSb = getSupabaseConfig();
      setSbConfig(currentSb);
            ...
            {activeTab === 'supabase' && (
              <div className="space-y-5 animate-fade-in">
                <div className="border-b border-slate-200 pb-3 flex items-center justify-between">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                    sbConfig.enabled ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600'
                  }`}>
                ...
                <div className="p-4 bg-emerald-950 text-emerald-100 rounded-xl space-y-2 border border-emerald-800">
                  <div className="flex items-center justify-between text-xs font-bold text-emerald-300">
                    ...
                    <span className="bg-emerald-500/20 text-emerald-200 border border-emerald-500/30 px-2 py-0.5 rounded text-[10px] font-mono">
                      Filter RLS: tenant_domain = eq.{getActiveDomainSlug(activeTestingDomain || undefined)}
                    </span>
                  </div>
                  <p className="text-[11px] text-emerald-200 leading-relaxed">
                    Setiap domain sekolah terisolasi secara total di dalam PostgreSQL menggunakan kolom <code className="bg-emerald-900 text-emerald-300 px-1 py-0.5 rounded font-mono">tenant_domain</code> dan tabel terpartisi. Tidak ada risiko kebocoran data antar-sekolah.
                  </p>
                </div>
                ...
                <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl flex items-center justify-between gap-4">
                  <div className="space-y-0.5">
                    <span className="font-bold text-xs text-emerald-950 block">Status Sync Supabase:</span>
                    <span className="text-[11px] text-slate-600">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 text-xs font-mono rounded-lg border border-slate-300 focus:ring-2 focus:ring-emerald-500"
                    />
                  </div>
